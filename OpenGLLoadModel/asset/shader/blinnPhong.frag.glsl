...
{
    return max(0.04 * (1.0 - dot(fs_in.normal, light_direction)), 0.0005);
}

void poisson_disk_samples(const vec2 random_seed)
{
    float angle_step = PI2 * float(RING_COUNT) / float(SAMPLE_COUNT);
    float inv_sample_count = 1.0 / float(SAMPLE_COUNT);
    float angle = random_0_to_1(random_seed) * PI2;
...
float shadow_calculation_use_shadow_map(const vec3 light_direction)
{
    ...
    vec3 projection_coords = fs_in.position_light_space.xyz / fs_in.position_light_space.w;
    projection_coords = projection_coords * 0.5 + 0.5;
    float map_depth = texture(depth_map, projection_coords.xy).r;
    return projection_coords.z > map_depth + bias ? 1.0 : 0.0;
}
...
float shadow_calculation_use_PCF(const vec3 light_direction)
{
    ...
    for (int i = 0; i < SAMPLE_COUNT; ++i)
    {
        ...
        shadow += projection_coords.z > map_depth + bias ? 1.0 : 0.0;
    }
...
void main()
{
    ...
    vec3 ambient_color = material.Ka_map == 1 ? pow(texture(material.Ka_texture, fs_in.texture_coord).rgb, vec3(gamma)) : material.Ka;
    vec3 ambient = 0.05 * ambient_color * attenuation;

    vec3 light_direction = normalize(light.position - fs_in.position);
    vec3 normal = normalize(fs_in.normal);
    float diffuse_coeff = max(dot(light_direction, normal), 0.0);
    vec3 diffuse_color = material.Kd_map == 1 ? pow(texture(material.Kd_texture, fs_in.texture_coord).rgb, vec3(gamma)) : material.Kd;
    ...
    vec3 specular_color = material.Ks_map == 1 ? pow(texture(material.Ks_texture, fs_in.texture_coord).rgb, vec3(gamma)) : material.Ks;
